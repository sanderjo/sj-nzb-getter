...
if __name__ == '__main__':
	...
	# Fill queue with articles from the NZB, which then can be fetched:
	my_nzb = open(nzbfilename).read()
	files = nzb_parser.parse(my_nzb)
	q = multiprocessing.Queue()	# a FIFO queue
	for nzb_file in files:
	    print "subject is", nzb_file.subject
	...
	for i in range(numberofprocesses):
		q.put(None)	# it's a FIFO queue, so this None will be get() after all the articles

	# ... and wait for processes to really stop
	#for job in jobs:
	#	job.join()
